...
- Link unfurls: `/s/:shareId` og:image prefers the animated GIF summary (`gifPath`, frames sampled across the whole trimmed clip; auto-generated during publish when the editor didn't supply one), falling back to the JPEG thumbnail.
- Timeline comments: `recording_comments` table + `/api/recordings/:shareId/comments` CRUD (view = anyone who can view the recording; post = signed-in; edit = author; resolve/delete = author or recording owner). Public view shows open comments by default with markers on the scrub bar; a signed-out visitor's draft survives the sign-in round trip via localStorage. Requires `pnpm --filter @workspace/db run push` on deploy.
- Downloads: library "Download" hands over the combined recording file (composite already bakes in camera/cursor/clicks) plus `.srt`/`.vtt` caption files; cloud-only items use `/play?download=1` (signed attachment URL). New recordings prefer native MP4 (H.264/AAC) when the browser's MediaRecorder supports it, WebM otherwise.
- Desktop share picker: the Mac app uses an in-app Zoom-style screen/window picker (desktopCapturer via `capture:list`/`capture:select` IPC) instead of the OS picker; window captures disable the cursor-overlay mapping. Camera size (none/small/large/full-screen) is live-adjustable during recording via the same mutable-var mechanism as the selfie corner (`camera:*` RecorderCommands from the presenter overlay).
- Local-first: all recordings live in IndexedDB (video blob + metadata, with capture chunks streamed to a `chunks` store during recording to bound memory). Uploads follow a server-authoritative lifecycle: `POST /api/recordings/start` creates a `pending` row (visible/resumable across devices), media transfers directly to object storage (resumable session persisted locally so a reload resumes mid-file; unfinished uploads auto-restart at app boot), then `POST /api/recordings/:shareId/complete` verifies the stored object and flips the row to `ready`. The transcript syncs via a separate `PATCH .../transcript` so the metadata commit stays small. `GET /api/recordings` is the server-side library, merged into the Library grid as cloud-only cards. Playback uses `GET /api/recordings/:shareId/play` (short-lived signed storage URL — bytes never proxy through Express); thumbnails/GIFs still use the `/api/storage` proxy. A server-side sweeper reaps stale pending rows and orphaned storage objects (48h window), and deleting a recording deletes its objects. Schema changes (e.g. the `status` column) apply via `pnpm --filter @workspace/db run push` on deploy.

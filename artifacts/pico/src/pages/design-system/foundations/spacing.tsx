      ...
      <div className="space-y-6">
        {spaces.map((space, i) => (
          <div key={space.token} className="flex items-center gap-6 group">
            ...
            <div className="flex-1 max-w-md h-12 bg-primary/10 border-2 border-foreground/20 rounded-sm flex items-center px-1 overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${space.px}px` }}
                transition={{ duration: 0.5, delay: i * 0.06 }}
                className="h-10 bg-primary border-2 border-foreground shrink-0"
              />
            </div>
